...
NFA::NFA(int Q, int S, int A, int M) {
    ...
    for(i = 0; i < m; i++){
        ...
        if(symbol == '$') {
            ...
            if(!lambdaClosureHold[currentState].empty()) for(itSet = lambdaClosureHold[currentState].begin(); itSet != lambdaClosureHold[currentState].end(); itSet++) previous(currentState, *itSet);
        }
        else {
...
void NFA::previous(int current, int pre) {

    std::set<int> hold1,hold2;
    hold1.clear();
    hold2.clear();
    std::set<int>::iterator itSet;
    hold1 = lambdaClosure[current];
    hold2 = lambdaClosure[pre];
    lambdaClosure[pre].merge(hold1);
    if(hold2 == lambdaClosure[pre]) return;
    if(!lambdaClosureHold[pre].empty()) for(itSet = lambdaClosureHold[pre].begin(); itSet != lambdaClosureHold[pre].end(); itSet++) previous(pre, *itSet);
